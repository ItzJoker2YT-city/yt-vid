...
def _fetch_html(url: str) -> str:
    ...
    resp = _requests.get(url, headers=headers, timeout=PAGE_TIMEOUT)
...
def get_ghana_songs_cached(page: int = 1, limit: int = _PER_PAGE, force_raw: bool = False) -> list:
    ...
    cache = _load_cache()
    if force_raw or not cache["songs"]:
        logger.info("Cache empty / forced: rebuilding...")
        cache = build_cache(max_pages=5)
