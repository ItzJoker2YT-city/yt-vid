...
def stream_and_delete(file_path, original_tasks=None):
    """
...
def api_download_file(task_id):
    ...
    response = Response(stream_and_delete(task.filepath), headers={
        "Content-Disposition": f"attachment; filename=\"{task.filename}\"",
        "Content-Type": mime_type or "application/octet-stream"
    })
    
    # We clear the filepath on the task object so the UI updates
    # (Note: it will be physically deleted by the generator's finally block)
    task.filepath = ""
    return response
